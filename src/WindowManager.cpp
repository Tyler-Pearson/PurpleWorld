...
bool WindowManager::init(int const width, int const height)
{
	...
	// Switch to this for fullscreen mode
	windowHandle = glfwCreateWindow(width, height, "Camera", glfwGetPrimaryMonitor(), nullptr);
	// windowHandle = glfwCreateWindow(width, height, "Camera", nullptr, nullptr);
	if (! windowHandle)
	{
		glfwTerminate();
		return false;
